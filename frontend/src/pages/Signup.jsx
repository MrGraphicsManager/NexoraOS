      ...
      {step === 1 ? (
        <form onSubmit={sendOtp} className="space-y-4">
          <Field label="Your name">
            <input data-testid="signup-name-input" required value={form.name} onChange={upd("name")} className="input" placeholder="Priya Kumar"/>
          </Field>
          <Field label="Café name">
            <input data-testid="signup-cafe-input" required value={form.cafe_name} onChange={upd("cafe_name")} className="input" placeholder="Nexora Café"/>
          </Field>
          <Field label="Email">
            <input data-testid="signup-email-input" type="email" required value={form.email} onChange={upd("email")} className="input" placeholder="owner@cafe.com"/>
          </Field>
          <Field label="Password (6+ chars)">
            <input data-testid="signup-password-input" type="password" required minLength={6} value={form.password} onChange={upd("password")} className="input"/>
          </Field>
          ...
          <div className="text-sm text-[#6B5A52] text-center">Already have an account? <Link to="/login" className="text-[#C85A32] font-semibold hover:underline">Sign in</Link></div>
        </form>
      ) : (
        <form onSubmit={verify} className="space-y-4">
          <Field label="6-digit code">
            <input data-testid="signup-otp-input" required maxLength={6} pattern="\d{6}" value={otp} onChange={(e)=>setOtp(e.target.value)} className="input tabular text-center text-2xl tracking-[0.5em] font-semibold" placeholder="000000"/>
          </Field>
